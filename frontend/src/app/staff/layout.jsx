    ...
    useEffect(() => {
        ...
        if (userCookie) {
            try {
                ...
                // ตรวจสอบว่าเป็น staff หรือไม่ (รองรับทั้ง uppercase และ lowercase)
                    ...
                    <Dropdown menu={{ items: userMenuItems }} placement="bottomRight">
                        <div style={{ display: "flex", alignItems: "center", cursor: "pointer", height: 64, padding: "0 12px", borderRadius: 6 }} className="hover:bg-gray-50 transition-colors">
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", marginRight: 12, lineHeight: 1.3 }}>
                                <Text strong style={{ fontSize: 14 }}>{user.firstName || user.first_name} {user.lastName || user.last_name}</Text>
                                <Text type="secondary" style={{ fontSize: 11 }}>
                                    {user.role?.toUpperCase() === "STAFF" ? "เจ้าหน้าที่" : user.role}
                                </Text>
                            </div>
